...
Device::Device(int busTypeMask,unsigned short sVendorId,unsigned short sProductId,unsigned int index)
	...
	udev_list_entry* dlPtr=enumerator.getListEntry();
	while(dlPtr!=0)
		{
		/* Get the udev device for the current list entry: */
		UdevDevice rawhid=context.getDeviceFromSyspath(udev_list_entry_get_name(dlPtr));
		...
		/* Go to the next device list entry: */
		dlPtr=udev_list_entry_get_next(dlPtr);
		}
